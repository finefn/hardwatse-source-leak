...
static const char* aimkeyandshit[]
{
	"RMB",
	"LMB",
	"SHIFT",
	...
	VOID EndScene(ImGuiWindow& window)
	{
		...
		/*if (Settings.TeleportToEnemies) //boat fly
		{
			if (hooks::LocalPlayerController)
			{
				if (InVehicle)
				{
					auto LocPtr = hooks::GetPawnRootLocation(FortniteHooking::ClosestVehicle);
					auto TpPos = *LocPtr;

					if (FortniteUtils::SpoofCall(Util::SpoofCall(GetAsyncKeyState, 0x57)) { // W
						TpPos.X = TpPos.X + Settings.BoatFlySpeed;
						// move to front
					}
					if (FortniteUtils::SpoofCall(Util::SpoofCall(GetAsyncKeyState, 0x53)) { // S
						TpPos.X = TpPos.X - Settings.BoatFlySpeed;
						// move to back
					}
					if (FortniteUtils::SpoofCall(Util::SpoofCall(GetAsyncKeyState, 0x41)) { // A
						TpPos.Y = TpPos.Y + Settings.BoatFlySpeed;
						// move to left
					}
					if (FortniteUtils::SpoofCall(Util::SpoofCall(GetAsyncKeyState, 0x44)) { // D
						TpPos.Y = TpPos.Y - Settings.BoatFlySpeed;
						// move to right
					}
					...
					FortniteHooking::ProcessEvent(FortniteHooking::ClosestVehicle, addresses::LaunchCharacter, &TpPos, 0);
				}
			}
		}*/
ImGui::PopStyleColor();
...
if (showMenu)
{
	...
	colors[ImGuiCol_WindowBg] = ImVec4(0, 0.5, 1, 1);
	colors[ImGuiCol_ChildBg] = ImVec4(0, 0.5, 1, 1);
	colors[ImGuiCol_PopupBg] = ImVec4(0.08f, 0.08f, 0.08f, 0.94f);
	colors[ImGuiCol_Border] = ImVec4(1.00f, 1, 1, 1);
	colors[ImGuiCol_BorderShadow] = ImVec4(1, 1, 1, 1);
	colors[ImGuiCol_FrameBg] = ImVec4(0, 0.87f, 1, 0.6f);
	colors[ImGuiCol_FrameBgHovered] = ImVec4(0, 0.87f, 1, 0.64f);
	colors[ImGuiCol_FrameBgActive] = ImVec4(0, 0.87f, 1, 0.68f);
	colors[ImGuiCol_TitleBg] = ImVec4(0, 0.8, 1, 1.0f);
	colors[ImGuiCol_TitleBgActive] = ImVec4(0, 0.8, 1, 1.0f);
	colors[ImGuiCol_TitleBgCollapsed] = ImVec4(0, 0.8, 1, 1.0f);
	colors[ImGuiCol_MenuBarBg] = ImVec4(0.14f, 0.14f, 0.14f, 1.00f);
	colors[ImGuiCol_ScrollbarBg] = ImVec4(0.02f, 0.02f, 0.02f, 0);
	colors[ImGuiCol_ScrollbarGrab] = ImVec4(0.46f, 0.00f, 0.00f, 0);
	colors[ImGuiCol_ScrollbarGrabHovered] = ImVec4(1.00f, 0.00f, 0.00f, 0);
	colors[ImGuiCol_ScrollbarGrabActive] = ImVec4(0.46f, 0.00f, 0.00f, 0);
	colors[ImGuiCol_CheckMark] = ImVec4(1, 1, 1, 1);
	colors[ImGuiCol_SliderGrab] = ImVec4(1, 1, 1, 1);
	colors[ImGuiCol_SliderGrabActive] = ImVec4(1, 1, 1, 1);
	colors[ImGuiCol_Button] = ImVec4(0, 0.8f, 1, 1);
	colors[ImGuiCol_ButtonHovered] = ImVec4(0, 0.83f, 1, 1);
	colors[ImGuiCol_ButtonActive] = ImVec4(0, 0.87f, 1, 1);
	colors[ImGuiCol_Header] = ImVec4(0, 0.8f, 1, 1);
	colors[ImGuiCol_HeaderHovered] = ImVec4(0, 0.83f, 1, 1);
	colors[ImGuiCol_HeaderActive] = ImVec4(0, 0.87f, 1, 1);
	colors[ImGuiCol_Separator] = ImVec4(0, 0.8f, 1, 1);
	colors[ImGuiCol_SeparatorHovered] = ImVec4(0, 0.83f, 1, 1.f);
	colors[ImGuiCol_SeparatorActive] = ImVec4(0, 0.87f, 1, 1);
	...
	ImGui::GetStyle().FramePadding = ImVec2(3.0f, 1.8f);
	if (ImGui::Begin(xorstr("Hardwatse [FULL]"), 0, ImGuiWindowFlags_::ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_::ImGuiWindowFlags_NoResize)) {
		ImGui::SetWindowSize(ImVec2(304, 238.0f));
		static int tabs = 0;
		if (ImGui::Button("Aimbot", ImVec2(95.0f, 20.f)))
			tabs = 0; ImGui::SameLine(NULL, 0.8);
		if (ImGui::Button("ESP", ImVec2(95.0f, 20.f)))
			tabs = 1; ImGui::SameLine(NULL, 0.8);
		if (ImGui::Button("Exploits", ImVec2(95.0f, 20.f)))
			tabs = 2;
		switch (tabs)
		{
		case 0:
			...
			else if (clickedH && !Settings.Silent) {
			...
			if (clickedB && Settings.Silent) {
			...
			else if (clickedB && !Settings.SilentAimbot) {
				...
				case 2:
					...
					ImGui::SliderFloat("Speed [x]", &Settings.customspeedhax, 1.f, 25.0f, ("%2.f"));
					ImGui::SliderFloat("Camera FOV", &Settings.FOV, 80.0f, 160.0f, ("%2.f"));
					if (ImGui::Button("Set normal FOV", ImVec2(196.0f, 29.0f))) {
						Settings.FOV = 100.0f;
					}
	...
	HRESULT __stdcall hk_present(IDXGISwapChain* pSwapChain, UINT sync_interval, UINT flags)
	{
		...
		if (!device) {
			...
			m_pFont = ImGui::GetIO().Fonts->AddFontFromFileTTF(xorstr("C:\\Windows\\Fonts\\impact.ttf"), 14.0f);

			if (m_pFont == NULL)
			{
				MessageBoxA(0, xorstr("couldn't find the font."), xorstr("HardwatseFN"), MB_ICONERROR);
				exit(0);
			}
		...
		window.DrawList->AddCircle(ImVec2(width / 2, height / 2), Settings.AimbotFOV, ImGui::GetColorU32({ FovColor }), 22); //128


		window.DrawList->AddLine(ImVec2(950, 540), ImVec2(970, 540), ImGui::GetColorU32({ 0, 0.8f, 1, 1 }), 0.9f);
		window.DrawList->AddLine(ImVec2(960, 530), ImVec2(960, 550), ImGui::GetColorU32({ 0, 0.8f, 1, 1 }), 0.9f);
		...
		auto FpsColor = ImGui::GetColorU32({ 0, 0.8f, 1, 1 });
		DrawNormalText(m_pFont, TextFormat(xorstr("FPS: %.1f"), ImGui::GetIO().Framerate), ImVec2(20, 20), 12.0f, FpsColor, false);
			...
			*/
						...
						if (Bot) {
							...
							if (Util::WorldToScreen(width, height, &Botroot.X)) {
								...
								{                                  //ImGui::GetColorU32({ color_red, color_green, color_blue, 255 });
									DrawOutlinedText(m_pFont, TextFormat(xorstr("Bot [%.0f m]"), dist), ImVec2(Botroot.X, Botroot.Y), 14.5f, ImGui::GetColorU32({ 255, 255, 255, 255 }), true);
								}
							}
						}
					}
			...
			for (auto pawn : playerPawns) {
						...
						}*/
				{
					...
					{
						auto abcc187ccc = ImGui::GetColorU32({ 1.0f, 1.0f, 1.0f, 1.0f });
						root[2] -= 10;
						auto end = *reinterpret_cast<FVector*>(root);
						if (Util::WorldToScreen(width, height, &end.X))
						{
							window.DrawList->AddLine(ImVec2(960, 1030), ImVec2(end.X, end.Y), abcc187ccc);
						}
					}
					if (Settings.killaura == true) {
						auto abccccc = ImGui::GetColorU32({ 1, 1, 1, 1.0f });
						//ImGui::GetOverlayDrawList()->AddText(ImVec2(80, 180), ImGui::GetColorU32({ 1, 0, 0, 1 }), "Pickaxe teleport ACTIVE [F2]");
						DrawNormalText(m_pFont, TextFormat(xorstr("Pickaxe teleport ACTIVE [F2]")), ImVec2(100, 200), 12.0f, abccccc, true);
					}
					float minX = FLT_MAX;
					float maxX = -FLT_MAX;
					float minY = FLT_MAX;
					...
					if (Settings.ESP.PlayerNames)
					{
						head[2] += 15;
						auto headPos = *reinterpret_cast<FVector*>(head);
						auto abccccc = ImGui::GetColorU32({ 1.0f, 1.0f, 1.0f, 1.0f });
					...
					if (Settings.ESP.PlayerWeapons)
					{
						...
						if (Util::WorldToScreen(width, height, &bottomPos.X))
						{
							...
							auto NameCodfghdfghlor = ImGui::GetColorU32({ 1.0f, 1.0f, 1.0f, 1.0f });
					...
					if (Settings.ESP.Skeletons)
					{
						...
						auto dfhgfghdsdfgh = ImGui::GetColorU32({ 0, 0.8f, 1, 1 });
				...
				if (Settings.ESP.PlayerAmmo)
				{
					...
					if (actorAmmoCount)
					{
						...
						if (Util::WorldToScreen(width, height, &bottomPos.X))
						{
							...
							auto NameCoddfghdfhgfghdfghlor = ImGui::GetColorU32({ 1.0f, 1.0f, 1.0f, 1.0f });
			...
			if (Settings.Aimbot)
			{
				// shift: 0x10 ; rmb: 0x02 ; lmb = 0x01
				if (niggggga == 0) { //rmb
					if (closestPawn && Util::SpoofCall(GetAsyncKeyState, 0x02) && Util::SpoofCall(GetForegroundWindow) == hWnd)
					{
				...
				if (niggggga == 1) { //lmb
					if (closestPawn && Util::SpoofCall(GetAsyncKeyState, 0x01) && Util::SpoofCall(GetForegroundWindow) == hWnd)
					{
						hooks::TargetPawn = closestPawn;
					}
					else
				...
				if (niggggga == 2) { //shift
					if (closestPawn && Util::SpoofCall(GetAsyncKeyState, 0x10) && Util::SpoofCall(GetForegroundWindow) == hWnd)
					{
						hooks::TargetPawn = closestPawn;
					}
					else
	...
	BOOLEAN Initialize()
	{
		...
		if (FAILED(D3D11CreateDeviceAndSwapChain(nullptr, D3D_DRIVER_TYPE_HARDWARE, 0, 0, &featureLevel, 1, D3D11_SDK_VERSION, &sd, &swapChain, &device, nullptr, &context))) {
			MessageBox(0, xorstr(L"failed: dx11"), xorstr(L"HardwatseFN"), MB_ICONERROR);
			return FALSE;
		}
